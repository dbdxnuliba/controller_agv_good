...
namespace bz_robot
{
class ServiceGetLocState : public ServiceBase
{
public:
    ServiceGetLocState(const std::string cmd, const std::string arg, const std::string ret):
    ...
    void execute_once(void *p_args)
    {
        nlohmann::json j_out;
        j_out["#CMD#"] = m_cmd;
        j_out["score"] = mp_task_client->call<int>("localization_status");
