...
namespace bz_robot
{
class ServiceGetName : public ServiceBase
{
public:
    ServiceGetName(const std::string cmd, const std::string arg, const std::string ret):
    ...
    void execute_once(void *p_args)
    {
        nlohmann::json j_out;
        j_out["#CMD#"] = m_cmd;
        j_out["name"] = mp_task_client->call<std::string>("robot_name");
