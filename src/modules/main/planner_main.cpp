...
#include "planner/frenet_polynomial_planner/frenet_polynomial_planner.h"
...
namespace bz_robot
{
    ...
    public:
        ...
        ReturnStatus import_config(const std::string& config_file)
        {
            ...
            try
            {
                ...
                if(i)
                {
                    ...
                    else if(name == "FRENET_POLYNOMIAL")
                    {
                        p_planner = std::make_shared<frenet_polynomial_planner::FrenetPolynomialPlanner>();
                    }
                    else if(name == "R_STAR")
                    {
        ...
        RetMsg<PathData> plan(const Msg<GridMapData>& map_data, const Msg<Pose<FLOAT_T>> &pose_start,
                              const Msg<Pose<FLOAT_T>> &pose_goal)
        {
...
//            }
        ...
        void register_service()
        {
            ...
            p_server->register_handler("stop", &PlannerServer::stop, this, 1);
            p_server->register_handler("is_stop", &PlannerServer::is_stop, this, 1);
        }
...
void planner_module_main(int argc, char **argv)
{
    ...
    global_planner_server.p_server = std::make_shared<thread_rpc::Server>(MACRO_STR(MSG_ID_SERVER_GLOBAL_PLAN), 2);
    local_planner_server.p_server = std::make_shared<thread_rpc::Server>(MACRO_STR(MSG_ID_SERVER_LOCAL_PLAN), 2);
