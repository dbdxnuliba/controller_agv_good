...
#include <ctime>
#include <memory>
#include "sample_optimizing_smoother/path_optimizer.hpp"
...
#include "sample_optimizing_smoother/solver/solver.hpp"
...
PathOptimizer::PathOptimizer(const State &start_state,
    ...
    //grid_map_(new Map{p_map}),
    mp_map(std::make_shared<Map>(p_map)),
    mp_configs(p_configs),
    collision_checker_(new CollisionChecker(p_map, p_configs)),
    reference_path_(new ReferencePath(p_configs)),
    vehicle_state_(new VehicleState{start_state, end_state, 0, 0})
{
    mp_configs->update_config();
}
...
bool PathOptimizer::solve(const std::vector<State> &reference_points, std::vector<State> *final_path) {
    if (mp_configs->FLAGS_enable_computation_time_output) std::cout << "------" << std::endl;
    ...
    auto reference_path_smoother = ReferencePathSmoother::create(mp_configs->FLAGS_smoothing_method,
                                                                 reference_points,
...
bool PathOptimizer::solveWithoutSmoothing(const std::vector<PathOptimizationNS::State> &reference_points,
    ...
    reference_path_->updateBounds(mp_map);
...
bool PathOptimizer::segmentSmoothedPath() {
    ...
    if (!isEqual(end_distance, 0)) {
        ...
        double search_delta_s = mp_configs->FLAGS_enable_exact_position ? 0.1 : 0.5;
    ...
    const double delta_s_smaller = mp_configs->FLAGS_enable_raw_output ? 0.15 : 0.5;
    const double delta_s_larger = mp_configs->FLAGS_enable_raw_output ? mp_configs->FLAGS_output_spacing : 1.0;
...
bool PathOptimizer::optimizePath(std::vector<State> *final_path) {
    // Solve problem.
    auto solver = OsqpSolver::create(mp_configs->FLAGS_optimization_method, *reference_path_, *vehicle_state_, size_, mp_configs);
    ...
    if (mp_configs->FLAGS_enable_raw_output) {
        double s{0};
        for (auto iter = final_path->begin(); iter != final_path->end(); ++iter) {
            if (iter != final_path->begin()) s += distance(*(iter - 1), *iter);
            iter->s = s;
            if (mp_configs->FLAGS_enable_collision_check && !collision_checker_->isSingleStateCollisionFreeImproved(*iter)) {
        ...
        final_path->clear();
        double delta_s = mp_configs->FLAGS_output_spacing;
        for (int i = 0; i * delta_s <= result_s.back(); ++i) {
            ...
            State tmp_state{x_s(tmp_s),
                            ...
                            tmp_s};
            if (mp_configs->FLAGS_enable_collision_check && !collision_checker_->isSingleStateCollisionFreeImproved(tmp_state)) {
