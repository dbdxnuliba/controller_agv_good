...
ReferencePathImpl::ReferencePathImpl(std::shared_ptr<Configs> p_configs) :
...
void ReferencePathImpl::updateBoundsImproved(std::shared_ptr<Map> p_map) {
    ...
    for (const auto &state : reference_states_) {
        // Circle centers.
        State
            c0(state.x + mp_configs->FLAGS_d1 * cos(state.z),
               state.y + mp_configs->FLAGS_d1 * sin(state.z),
               state.z),
            c1(state.x + mp_configs->FLAGS_d2 * cos(state.z),
               state.y + mp_configs->FLAGS_d2 * sin(state.z),
               state.z),
            c2(state.x + mp_configs->FLAGS_d3 * cos(state.z),
               state.y + mp_configs->FLAGS_d3 * sin(state.z),
               state.z),
            c3(state.x + mp_configs->FLAGS_d4 * cos(state.z),
               state.y + mp_configs->FLAGS_d4 * sin(state.z),
               state.z);
        auto c00 = getApproxState(state, c0, mp_configs->FLAGS_d1);
        auto c11 = getApproxState(state, c1, mp_configs->FLAGS_d2);
        auto c22 = getApproxState(state, c2, mp_configs->FLAGS_d3);
        auto c33 = getApproxState(state, c3, mp_configs->FLAGS_d4);
        // Calculate boundaries.
        auto clearance_0 = getClearanceWithDirectionStrict(c00, p_map);
        auto offset_0 = global2Local(c0, c00).y;
        clearance_0[0] += offset_0; clearance_0[1] += offset_0;

        auto clearance_1 = getClearanceWithDirectionStrict(c11, p_map);
        auto offset_1 = global2Local(c1, c11).y;
        clearance_1[0] += offset_1; clearance_1[1] += offset_1;

        auto clearance_2 = getClearanceWithDirectionStrict(c22, p_map);
        auto offset_2 = global2Local(c2, c22).y;
        clearance_2[0] += offset_2; clearance_2[1] += offset_2;

        auto clearance_3 = getClearanceWithDirectionStrict(c33, p_map);
...
void ReferencePathImpl::updateLimits() {
    ...
    if (mp_configs->FLAGS_optimization_method != "KPC") {
    ...
    if (use_spline_) {
        ...
        for (size_t i = 0; i != reference_states_.size(); ++i) {
            max_k_list_.emplace_back(tan(mp_configs->FLAGS_max_steering_angle) / mp_configs->FLAGS_wheel_base);
    ...
    for (size_t i = 0; i != reference_states_.size(); ++i) {
        ...
        double ay_allowed = sqrt(pow(mp_configs->FLAGS_mu * 9.8, 2) - pow(ref_ax, 2));
        if (ref_v > 0.0001) max_k_list_.emplace_back(ay_allowed / pow(ref_v, 2));
        else max_k_list_.emplace_back(DBL_MAX);
        // Control rate limit.
        if (ref_v > 0.0001) max_kp_list_.emplace_back(mp_configs->FLAGS_max_curvature_rate / ref_v);
        else max_kp_list_.emplace_back(DBL_MAX);
    }
    //LOG(INFO) << "K and KP constraints are updated according to v and a.";
}

void ReferencePathImpl::updateBounds(std::shared_ptr<PathOptimizationNS::Map> p_map) {
    if (reference_states_.empty()) {
        //LOG(WARNING) << "Empty reference, updateBounds fail!";
        return;
    }
    ...
    for (const auto &state : reference_states_) {
        ...
        State
            ...
            c3(state.x + mp_configs->FLAGS_d4 * cos(state.z),
               ...
               state.z);
        // Calculate boundaries.
        auto clearance_0 = getClearanceWithDirectionStrict(c0, p_map);
        auto clearance_1 = getClearanceWithDirectionStrict(c1, p_map);
        auto clearance_2 = getClearanceWithDirectionStrict(c2, p_map);
        auto clearance_3 = getClearanceWithDirectionStrict(c3, p_map);
...
std::vector<double> ReferencePathImpl::getClearanceWithDirectionStrict(const PathOptimizationNS::State &state,
                                                                       std::shared_ptr<PathOptimizationNS::Map> p_map)
{
    ...
    Eigen::Vector2d pos(state.x, state.y);
    auto original_clearance = p_map->getObstacleDistance(pos);
    if (original_clearance > mp_configs->FLAGS_circle_radius) {
        // Normal case:
        double right_s = 0;
        for (size_t j = 0; j != n; ++j) {
            ...
            Eigen::Vector2d new_position(x, y);
            double clearance = p_map->getObstacleDistance(new_position);
            if (clearance < mp_configs->FLAGS_circle_radius) {
                break;
            }
        }
        double left_s = 0;
    ...
    } else if (is_original_spline_set && use_spline_ && !mp_configs->FLAGS_enable_simple_boundary_decision) {
    ...
    for (int i = 1; i != static_cast<int>(delta_s / smaller_ds); ++i) {
        ...
        if (p_map->getObstacleDistance(position) < mp_configs->FLAGS_circle_radius) {
            left_bound -= smaller_ds;
            break;
        }
    }
...
bool ReferencePathImpl::buildReferenceFromSpline(double delta_s_smaller, double delta_s_larger) {
    ...
    while (tmp_s <= max_s_) {
        ...
        if (mp_configs->FLAGS_enable_dynamic_segmentation) {
