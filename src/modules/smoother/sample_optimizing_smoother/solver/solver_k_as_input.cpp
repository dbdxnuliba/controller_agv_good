...
#include "sample_optimizing_smoother/solver/solver_k_as_input.hpp"
...
void SolverKAsInput::setHessianMatrix(Eigen::SparseMatrix<double> *matrix_h) const {
    ...
    double w_c = mp_configs->FLAGS_K_curvature_weight;
    double w_cr = mp_configs->FLAGS_K_curvature_rate_weight;
    double w_pq = mp_configs->FLAGS_K_deviation_weight;
    double w_e = mp_configs->FLAGS_KP_slack_weight;
...
void SolverKAsInput::setDynamicMatrix(size_t i,
    ...
    double ref_delta = atan(ref_k * mp_configs->FLAGS_wheel_base);
    Eigen::Matrix2d a;
    a << 1, -ref_s * pow(ref_k, 2),
        ref_s, 1;
    Eigen::Matrix<double, 2, 1> b;
    b << ref_s / mp_configs->FLAGS_wheel_base / pow(cos(ref_delta), 2), 0;
    *matrix_a = a;
    *matrix_b = b;
}
...
void SolverKAsInput::setConstraintMatrix(Eigen::SparseMatrix<double> *matrix_constraints,
    ...
    collision << mp_configs->FLAGS_d1, 1,
//        mp_configs->FLAGS_d2, 1,
        mp_configs->FLAGS_d3, 1,
        mp_configs->FLAGS_d4, 1;
    for (size_t i = 0; i != horizon_; ++i) {
    ...
    collision1 << mp_configs->FLAGS_d2, 1;
    for (size_t i = 0; i != horizon_; ++i) {
    ...
    *upper_bound = Eigen::MatrixXd::Zero(11 * horizon_ - 1, 1);
    ...
    for (size_t i = 0; i != horizon_ - 1; ++i) {
        ...
        double steer = atan(ref_states[i].k * mp_configs->FLAGS_wheel_base);
        Eigen::Vector2d c;
        c << ds * steer / mp_configs->FLAGS_wheel_base / pow(cos(steer), 2), 0;
    ...
    lower_bound->block(4 * horizon_, 0, horizon_ - 1, 1) =
        Eigen::VectorXd::Constant(horizon_ - 1, -mp_configs->FLAGS_max_steering_angle);
    ...
    upper_bound->block(5 * horizon_ - 1, 0, horizon_, 1) =
        Eigen::VectorXd::Constant(horizon_, mp_configs->FLAGS_expected_safety_margin);
    ...
    for (size_t i = 0; i != horizon_; ++i) {
        double ud = bounds[i].c1.ub - mp_configs->FLAGS_expected_safety_margin;
        double ld = bounds[i].c1.lb + mp_configs->FLAGS_expected_safety_margin;
