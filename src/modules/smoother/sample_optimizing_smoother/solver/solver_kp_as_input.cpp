...
//        1, mp_configs->FLAGS_d3,
    ...
    *upper_bound = Eigen::MatrixXd::Zero(10 * horizon_ + control_horizon_ + 2, 1);
    ...
    // Vars bound.
    for (size_t i = 0; i != horizon_; ++i) {
        (*lower_bound)(vars_range_begin + i) = -tan(mp_configs->FLAGS_max_steering_angle) / mp_configs->FLAGS_wheel_base;
        (*upper_bound)(vars_range_begin + i) = tan(mp_configs->FLAGS_max_steering_angle) / mp_configs->FLAGS_wheel_base;
        (*lower_bound)(vars_range_begin + horizon_ + control_horizon_ + i) = 0;
        (*upper_bound)(vars_range_begin + horizon_ + control_horizon_ + i) = mp_configs->FLAGS_expected_safety_margin;
    }
    for (size_t i = 0; i != control_horizon_; ++i) {
