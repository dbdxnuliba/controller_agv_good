...
#include "sample_optimizing_smoother/solver/solver_kp_as_input.hpp"
...
std::unique_ptr<OsqpSolver> OsqpSolver::create(std::string &type,
                                               ...
                                               const size_t &horizon, std::shared_ptr<Configs> p_configs) {
    if (type == "K") {
        return std::unique_ptr<OsqpSolver>(new SolverKAsInput(reference_path, vehicle_state, horizon, p_configs));
    } else if (type == "KP") {
        return std::unique_ptr<OsqpSolver>(new SolverKpAsInput(reference_path, vehicle_state, horizon, p_configs));
    } else if (type == "KPC") {
        return std::unique_ptr<OsqpSolver>(new SolverKpAsInputConstrained(reference_path, vehicle_state, horizon, p_configs));
    } else {
