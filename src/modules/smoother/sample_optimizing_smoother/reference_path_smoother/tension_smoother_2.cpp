...
FgEvalQPSmoothing::FgEvalQPSmoothing(const std::vector<double> &seg_x_list,
    ...
    FgEvalReferenceSmoothing(seg_x_list, seg_y_list, seg_s_list, seg_angle_list, p_configs),
...
void FgEvalQPSmoothing::operator()(PathOptimizationNS::FgEvalReferenceSmoothing::ADvector &fg,
    ...
    for (size_t i = 0; i != point_num - 1; ++i) {
        ...
        // cost
        fg[0] += mp_configs->FLAGS_tension_2_deviation_weight * (pow(cur_x - ref_x, 2) + pow(cur_y - ref_y, 2));
        fg[0] += mp_configs->FLAGS_tension_2_curvature_weight * pow(cur_k, 2);
        if (i != 0) {
            ad pre_k = vars[k_idx_begin + i - 1];
            fg[0] += mp_configs->FLAGS_tension_2_curvature_rate_weight * pow(cur_k - pre_k, 2);
        }
        // cons
...
TensionSmoother2::TensionSmoother2(const std::vector<PathOptimizationNS::State> &input_points,
                                   ...
                                   std::shared_ptr<Configs> p_configs) :
    TensionSmoother(input_points, start_state, p_grid_map, p_configs) {}
...
bool TensionSmoother2::ipoptSmooth(const std::vector<double> &x_list,
    ...
    for (size_t i = 0; i < point_num; i++) {
        ...
        if (i != point_num - 1) {
            vars_lowerbound[k_idx_begin + i] = -0.3;//-tan(mp_configs->FLAGS_max_steering_angle) / mp_configs->FLAGS_wheel_base;
            vars_upperbound[k_idx_begin + i] = 0.3; //tan(mp_configs->FLAGS_max_steering_angle) / mp_configs->FLAGS_wheel_base;
        }
    }
...
void TensionSmoother2::setHessianMatrix(size_t size, Eigen::SparseMatrix<double> *matrix_h) const {
    ...
    for (int i = 0; i != size; ++i) {
        hessian(x_start_index + i, x_start_index + i) = hessian(y_start_index + i, y_start_index + i)
            = mp_configs->FLAGS_tension_2_deviation_weight * 2;
        if (i != size - 1) hessian(k_start_index + i, k_start_index + i) = mp_configs->FLAGS_tension_2_curvature_weight * 2;
    }
    // Curvature change.
    Eigen::Vector2d coeff_vec{1, -1};
    Eigen::Matrix2d coeff = coeff_vec * coeff_vec.transpose();
    for (int i = 0; i != size - 2; ++i) {
        hessian.block(k_start_index + i, k_start_index + i, 2, 2) += 2 * mp_configs->FLAGS_tension_2_curvature_rate_weight * coeff;
    }
...
void TensionSmoother2::setGradient(const std::vector<double> &x_list,
    ...
    *gradient = Eigen::VectorXd::Constant(4 * size - 1, 0);
    for (int i = 0; i != size; ++i) {
        (*gradient)(x_start_index + i) = -2 * mp_configs->FLAGS_tension_2_deviation_weight * x_list[i];
        (*gradient)(y_start_index + i) = -2 * mp_configs->FLAGS_tension_2_deviation_weight * y_list[i];
    }
}
}
}
