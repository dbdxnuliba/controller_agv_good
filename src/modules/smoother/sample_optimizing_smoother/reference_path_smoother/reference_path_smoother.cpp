...
#include "sample_optimizing_smoother/reference_path_smoother/angle_diff_smoother.hpp"
#include "sample_optimizing_smoother/reference_path_smoother/tension_smoother.hpp"
#include "sample_optimizing_smoother/reference_path_smoother/tension_smoother_2.hpp"
...
std::unique_ptr<ReferencePathSmoother> ReferencePathSmoother::create(const std::string &type,
    ...
    if (type == "ANGLE_DIFF") {
        return std::unique_ptr<ReferencePathSmoother>{new AngleDiffSmoother(input_points, start_state, p_grid_map, p_configs)};
    } else if (type == "TENSION") {
        return std::unique_ptr<ReferencePathSmoother>{new TensionSmoother(input_points, start_state, p_grid_map, p_configs)};
    } else if (type == "TENSION2") {
        return std::unique_ptr<ReferencePathSmoother>{new TensionSmoother2(input_points, start_state, p_grid_map, p_configs)};
    } else {
...
bool ReferencePathSmoother::solve(PathOptimizationNS::ReferencePath *reference_path,
    ...
    bSpline();
    if (mp_configs->FLAGS_enable_searching && graphSearch()) {
...
double ReferencePathSmoother::getG(const PathOptimizationNS::APoint &point,
    ...
    double distance_to_obs = mp_grid_map->getObstacleDistance(position);
    double safety_distance = 5;
    if (distance_to_obs < safety_distance) {
        obstacle_cost = (safety_distance - distance_to_obs) / safety_distance * mp_configs->FLAGS_search_obstacle_cost;
    }
    // Deviation cost.
    double offset_cost = fabs(point.offset) / mp_configs->FLAGS_search_lateral_range * mp_configs->FLAGS_search_deviation_cost;
    // Smoothness cost.
...
bool ReferencePathSmoother::graphSearch() {
    ...
    while (tmp_s < s_list_.back()) {
        ...
        tmp_s += mp_configs->FLAGS_search_longitudial_spacing;
    }
    ...
    for (size_t i = 1; i != layers_s_list.size(); ++i) {
        ...
        double left_range = mp_configs->FLAGS_search_lateral_range, right_range = -mp_configs->FLAGS_search_lateral_range;
        if (rr > 0) {
            // Left turn
        ...
        while (offset <= left_range) {
            ...
            point.offset = offset;
            Eigen::Vector2d position(point.x, point.y);
            if (mp_grid_map->isInside(position)
                && mp_grid_map->getObstacleDistance(position) > mp_configs->FLAGS_circle_radius) {
                point_set.emplace_back(point);
            }
            offset += mp_configs->FLAGS_search_lateral_spacing;
        }
    ...
    auto n = std::max(static_cast<int>(4.5 / mp_configs->FLAGS_search_longitudial_spacing), 1);
    ...
    auto t2 = std::clock();
    if (mp_configs->FLAGS_enable_computation_time_output) {
