...
void FgEvalReferenceSmoothing::operator()(PathOptimizationNS::FgEvalReferenceSmoothing::ADvector &fg,
    ...
    for (size_t i = 1; i != point_num - 1; ++i) {
        ...
        // Deviation cost:
        fg[0] += mp_configs->FLAGS_cartesian_deviation_weight * (pow(current_offset, 2));
        // Curvature cost:
        fg[0] += mp_configs->FLAGS_cartesian_curvature_weight
        ...
        if (i > 1) {
            ...
            fg[0] += mp_configs->FLAGS_cartesian_curvature_rate_weight *
...
TensionSmoother::TensionSmoother(const std::vector<PathOptimizationNS::State> &input_points,
    ...
    ReferencePathSmoother(input_points, start_state, p_grid_map, p_configs) {}
...
bool TensionSmoother::smooth(PathOptimizationNS::ReferencePath *reference_path,
    ...
    if (mp_configs->FLAGS_tension_solver == "IPOPT") {
        solver_ok = ipoptSmooth(x_list,
                                ...
                                &result_y_list,
                                &result_s_list);
    } else if (mp_configs->FLAGS_tension_solver == "OSQP") {
...
bool TensionSmoother::ipoptSmooth(const std::vector<double> &x_list,
    ...
    for (size_t i = 1; i != n_vars - 1; ++i) {
        double x = x_list[i];
        double y = y_list[i];
        double clearance = mp_grid_map->getObstacleDistance(Eigen::Vector2d(x, y));
        // Adjust clearance.
        clearance = std::max(clearance, default_clearance);
//            isEqual(clearance, 0) ? default_clearance :
//                        clearance > 0.5 ? clearance - 0.5 : clearance;
//                    clearance > mp_configs->FLAGS_circle_radius ? clearance - mp_configs->FLAGS_circle_radius : clearance;
...
void TensionSmoother::setHessianMatrix(size_t size, Eigen::SparseMatrix<double> *matrix_h) const {
    ...
    Eigen::Matrix3d dds_part{dds_vec * dds_vec.transpose() * mp_configs->FLAGS_cartesian_curvature_weight};
    Eigen::Matrix<double, 4, 1> ddds_vec{-1, 3, -3, 1};
    Eigen::Matrix4d ddds_part{ddds_vec * ddds_vec.transpose() * mp_configs->FLAGS_cartesian_curvature_rate_weight};
    ...
    for (int i = 0; i != size; ++i) {
        hessian(d_start_index + i, d_start_index + i) = mp_configs->FLAGS_cartesian_deviation_weight;
    }
