...
void DynamicVoronoi::buildVoronoiFromImage(std::shared_ptr<MapBase> p_map)
{
    RECORD_TIME();
  sizeX = p_map->size_in_cells_x();
  sizeY = p_map->size_in_cells_y();
  ...
  for (uint32_t x = 0; x < sizeX; ++x) {
    for (uint32_t y = 0; y < sizeY; ++y) {
      //binMap[x][y] = p_map->cost(x, y) < obstacle_cost ? true : false;
      binMap[x][y] = p_map->cost(x, y) < obstacle_cost ? false : true;
    }
  }
...
void DynamicVoronoi::visualize(const char *filename) {
  ...
  //fputc() runs 3 times for assigning a pixel's RGB color in turn
  for(int y = sizeY-1; y >=0; y--){      
    for(int x = 0; x<sizeX; x++){	
      unsigned char c = 0;
      if (isVoronoi(x,y)) {  //Voronoi edge
        fputc( 255, F );
        fputc( 0, F );
        fputc( 0, F );
      } else if (data[x][y].sqdist==0) {  //obstacle
        fputc( 0, F );
        fputc( 0, F );
        fputc( 0, F );
      } else {    //inner space of Voronoi diagram
...
Vec2i DynamicVoronoi::GetClosestVoronoiEdgePoint(Vec2d xi, float& closest_dis) {
  Vec2i closest_pt;
  ...
  closest_dis = sqrt(static_cast<float>(closest_dis_sq));
