...
bool CubicSpline::compSplineParams()
{
    ...
    spline_params_.b.setZero(spline_data_.n_points);
    spline_params_.c.setZero(spline_data_.n_points);
    spline_params_.d.setZero(spline_data_.n_points);
    // 定义可以计算a,b,c,d值的变量
    ...
    spline_params_.b(spline_data_.n_points-1) = 0.0;
    spline_params_.c(spline_data_.n_points-1) = 0.0;
    spline_params_.d(spline_data_.n_points-1) = 0.0;
...
double CubicSpline::getPoint(double x) const
{
    ...
    // 保证到达终点时也把x置为0
    //debug x = max length
//    if(x == spline_data_.x_data(spline_data_.x_data.size() - 1))
//    {
//      std::cout<<"获取终点坐标"<<std::endl;
//    }
    x = unwrapInput(x);
    // 获取对应的编号
    index = getIndex(x);
