...
void MPC::WrapInput(const SplineParam &track_param)
{
  ...
  {
    last_result_.at(default_param_.x_start+i)   = end_point[0]+0.1*cos(end_point[2]);
    last_result_.at(default_param_.y_start+i)   = end_point[1]+0.1*sin(end_point[2]);
...
//    upper_bound[i] = var[i]+default_param_.max_speed*DT;
  ...
  {
    lower_bound[i] = default_param_.max_speed;
...
void MPC::SetSloverOption(std::string &option)
{
  ...
  option += "Numeric max_cpu_time          0.5\n";
}
