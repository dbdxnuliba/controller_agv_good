...
tracker_mpc::tracker_mpc()
{
  ...
  mpc_result_.result.resize(6*SIZE_PREDICTION-2,0.0);
}
...
void tracker_mpc::ComputeMpc()
{
  while (true)
  {
    PERIODIC_MS_TASK(DT*1000);
    //针对收到目标点但是还没有收到局部路径的情况
    if(!has_path_)
    {
      std::cout<<"收到目标点，但是还没有规划路径"<<std::endl;
      std::fill(mpc_result_.result.begin(),mpc_result_.result.end(),0.0);
      mpc_result_.result_num = -1;
      continue;
    }
    //1.循环结束的判断条件
    if(!is_compute_mpc_)
    {
      std::cout<<"到达终点，结束MPC计算"<<std::endl;
      std::fill(mpc_result_.result.begin(),mpc_result_.result.end(),0.0);
      mpc_result_.result_num = -1;
      break;
    }
    //2.计算主循环
    std::cout<<"**********MPC主循环**********"<<std::endl;
...
Msg<ControlData> tracker_mpc::scroll_calculate(const FLOAT_T &dt, const Msg<Pose<FLOAT_T> > &msg_location, const Msg<ControlData> &msg_cur_control_data)
{
  ...
  //如果收到零速，且经过了MPC控制器，说明是到达目标点，将参数初始化
  if(mpc_result_.result.at(default_param_.v_start+1)==0.0&&
     mpc_result_.result.at(default_param_.delta_start)==0.0)
  {
    Init();
    has_path_ = false;
...
Msg<bool> tracker_mpc::is_goal_reached()
{
  Msg<bool> msg;
...
bool tracker_mpc::set_stop_signal(const Msg<bool> &stop)
{
  is_compute_mpc_ = !stop.data;
  return true;
}
