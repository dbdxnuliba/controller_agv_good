#include "tracker_lqr.h"

namespace bz_robot
{

tracker_lqr::tracker_lqr()
{
  std::cout<<"开始LQR控制器初始化"<<std::endl;
  p_lqr_ = std::make_shared<LQR>();
  Init();
  goal_.data.position.x    = 0.0;
  goal_.data.position.y    = 0.0;
  goal_.data.heading_angle = 0.0;

  is_compute_lqr_ = false;
}
...
bool tracker_lqr::import_config(const char *file_path)
{
  try
  {
...
bool tracker_lqr::set_path(const Msg<PathData> &path, const Msg<ControlData> &msg_cur_control_data)
{
  if(path.data.size()<=1)
  {
  ...
  p_lqr_->SetTrack(path.data, msg_cur_control_data.data);
  last_path_ = path;
  return true;
}

bool tracker_lqr::set_goal(const Msg<Pose<FLOAT_T> > &goal)
{
  goal_ = goal;
  return true;
}

Msg<bool> tracker_lqr::is_goal_reached()
{
  Msg<bool> msg;
...
bool tracker_lqr::set_stop_signal(const Msg<bool> &stop)
{
  is_compute_lqr_ = !stop.data;
  return true;
}

Msg<ControlData> tracker_lqr::scroll_calculate(const FLOAT_T &dt, const Msg<Pose<FLOAT_T> > &msg_location, const Msg<ControlData> &msg_cur_control_data)
{
  Msg<ControlData> msg;
  msg.time_stamp_us = time_stamp_us();
  msg.data.dt = dt;
  if(reached_target_pose_)
  {
    msg.data.steer_angle = 0.0;
    msg.data.velocity    = 0.0;
    return msg;
  }
  ControlData control_param = p_lqr_->ComputeLqr(msg_location.data,msg_cur_control_data.data,goal_);
...
bool tracker_lqr::IsNeedSetPath(const Msg<PathData> &last_path, const Msg<PathData> &current_path)
{
