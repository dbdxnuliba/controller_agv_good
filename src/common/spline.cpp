...
namespace SplineSpace
{
				...
				:GivenX(x0),GivenY(y0),GivenNum(num),Bc(bc),LeftB(leftBoundary),RightB(rightBoundary)
	{
        ...
        PartialDerivative = new float[GivenNum];	//给偏导分配空间
	...
	{
//  追赶法解方程求二阶偏导数
        float *a=new float[GivenNum];                //  a:稀疏矩阵最下边一串数
        float *b=new float[GivenNum];                //  b:稀疏矩阵最中间一串数
        float *c=new float[GivenNum];                //  c:稀疏矩阵最上边一串数
...
//  追赶法解方程求二阶偏导数
	...
	//单个插值的实现
    bool Spline::SinglePointInterp(const float& x,float& y)throw(SplineFailure)
	{
		if((x<MinX)|(x>MaxX))
	...
	//多个插值的实现
    bool Spline::MultiPointInterp(const float* x,const int& num,float* y)throw(SplineFailure)
	{
		for(int i = 0;i < num;i++)
		{
	...
	//自动多个插值的实现
    bool Spline::AutoInterp(const int& num,float* x,float* y)throw(SplineFailure)
	{
		if(num < 2)
			throw SplineFailure("至少要输出两个点");
        float perStep = (MaxX-MinX)/(num-1);
