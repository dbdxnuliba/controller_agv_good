...
namespace SplineSpace		
{
	...
	public:
		...
		//y->返回插值结果(输出)
		//如果x不是内插值点则插值失败
        virtual bool SinglePointInterp(const float& x,float& y)=0;
		//多点插值:
		//x->输入自变量数组首指针,num->自变量个数的引用(输入)
		//y->返回插值结果的首指针(输出数组)
        virtual bool MultiPointInterp(const float* x,const int& num,float* y)=0;
		//自动插值:
		//num->需要生成的插值个数的引用(输入)
		//x->存放自变量数组的首指针(输出数组)
		//y->对应拟合结果的首指针(输出数组)
        virtual bool AutoInterp(const int& num,float* x,float* y)=0;
	...
	public:
        ...
        Spline(const float* x0,const float* y0,const int& num,
            BoundaryCondition bc=GivenSecondOrder,const float& leftBoundary=0,const float& rightBoundary=0);

        bool SinglePointInterp(const float& x,float& y)throw(SplineFailure);		//虚函数实现
        bool MultiPointInterp(const float* x,const int& num,float* y)throw(SplineFailure);
        bool AutoInterp(const int& num,float* x,float* y)throw(SplineFailure);
	...
	private:
		...
		//属性
        const float* GivenX;	//已知数据的自变量
        const float* GivenY;	//已知数据的因变量
		const int GivenNum;		//给定的已知数据点数
		const BoundaryCondition Bc;	//边界类型
        const float LeftB;
        const float RightB;

        float* PartialDerivative;	//保存对应的二阶或者一阶偏导数
        float MaxX;				//自变量的最大值
        float MinX;				//自变量的最小值

        const float* SplineX;	//插值自变量
        float* SplineY;		//插值因变量
