...
class ControlThread (threading.Thread):
    ...
    def run(self):
        ...
        while(True):
            recv_data = self.sub.recv()
            #print(recv_data)
            velocity = struct.unpack('<f', recv_data[-9:-5])[0]
            steer_angle = struct.unpack('<f', recv_data[-5:-1])[0] * 180 / math.pi
            MUTEX.acquire()
...
class TrackPoseThread(threading.Thread):
    ...
    def run(self):
        ...
        while(True):
            recv_data = self.sub.recv()
            x = struct.unpack('<f', recv_data[-13:-9])[0]
            y = struct.unpack('<f', recv_data[-9:-5])[0]
            angle = struct.unpack('<f', recv_data[-5:-1])[0] * 180 / math.pi
            pose = Pose(x, y, angle)
            MUTEX.acquire()
            if x != 0  or y != 0:
    ...
    def run(self):
        ...
        while(True):
            ...
            if path_size > 0:
                ...
                GLOBAL_PATH_Y_LIST = np.array([])
                GLOBAL_PATH_YAW_LIST = np.array([])
            for i in range(path_size//12):
                index = i * 12
                x = struct.unpack('<f', data[index:index+4])[0]
                y = struct.unpack('<f', data[index+4:index+8])[0]
                heading_angle = struct.unpack('<f', data[index+8:index+12])[0] * 180 / math.pi
...
class OdomThread (threading.Thread):
    ...
    def run(self):
        ...
        while(True):
            ...
            x = struct.unpack('<f', recv_data[-13:-9])[0]
            y = struct.unpack('<f', recv_data[-9:-5])[0]
            heading_angle = struct.unpack('<f', recv_data[-5:-1])[0] * 180 / math.pi
            MUTEX.acquire()
...
def update():
    ...
    plot_path.plot(PATH_X_LIST, PATH_Y_LIST, pen=None, symbol='x', symbolSize=10, symbolPen=(200,0,0,150), symbolBrush=(200,0,0,150))
