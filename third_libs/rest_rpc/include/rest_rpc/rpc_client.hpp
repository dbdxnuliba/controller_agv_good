...
#endif
        // zhou add
        // call without time_out
        template<typename T=void, typename... Args>
        typename std::enable_if<std::is_void<T>::value>::type call_without_timeout(const std::string& rpc_name, Args&& ... args) {
            std::future<req_result> future = async_call<FUTURE>(rpc_name, std::forward<Args>(args)...);
//            auto status = future.wait_for(std::chrono::milliseconds(TIMEOUT));
//            if (status == std::future_status::timeout || status == std::future_status::deferred) {
//                throw std::out_of_range("timeout or deferred");
...
//        template<typename T = void, typename... Args>
//        typename std::enable_if<std::is_void<T>::value>::type call_without_timeout(const std::string& rpc_name, Args&& ... args) {
//            call<T>(rpc_name, std::forward<Args>(args)...);
//        }

        template<typename T, typename... Args>
        typename std::enable_if<!std::is_void<T>::value, T>::type call_without_timeout(const std::string& rpc_name, Args&& ... args) {
            std::future<req_result> future = async_call<FUTURE>(rpc_name, std::forward<Args>(args)...);
//            auto status = future.wait_for(std::chrono::milliseconds(TIMEOUT));
//            if (status == std::future_status::timeout || status == std::future_status::deferred) {
//                throw std::out_of_range("timeout or deferred");
...
//        template<typename T, typename... Args>
//        typename std::enable_if<!std::is_void<T>::value, T>::type call_without_timeout(const std::string& rpc_name, Args&& ... args) {
//            return call_without_timeout<T>(rpc_name, std::forward<Args>(args)...);
//        }
