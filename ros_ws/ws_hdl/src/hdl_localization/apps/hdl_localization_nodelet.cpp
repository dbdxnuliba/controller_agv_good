...
private:
  ...
  void points_callback(const sensor_msgs::PointCloud2ConstPtr& points_msg) {
    ...
    // predict
    if(!use_imu) {
      pose_estimator->predict(stamp, Eigen::Vector3f::Zero(), Eigen::Vector3f::Zero(), Eigen::Quaternionf(1,0,0,0));
    } else {
      std::lock_guard<std::mutex> lock(imu_data_mutex);
      ...
      for(imu_iter; imu_iter != imu_data.end(); imu_iter++) {
        ...
        const auto& imu_quat = (*imu_iter)->orientation;    
        double gyro_sign = invert_imu ? -1.0 : 1.0;
        pose_estimator->predict((*imu_iter)->header.stamp, Eigen::Vector3f(acc.x, acc.y, acc.z), gyro_sign * Eigen::Vector3f(gyro.x, gyro.y, gyro.z),Eigen::Quaternionf(imu_quat.w,imu_quat.x,imu_quat.y,imu_quat.z));
      }
