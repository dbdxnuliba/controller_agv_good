...
public:
   ...
   * @param imu_quat quaternion
   */
  void predict(const ros::Time& stamp, const Eigen::Vector3f& acc, const Eigen::Vector3f& gyro, const Eigen::Quaternionf& imu_quat) {
    ...
    Eigen::VectorXf control(10);
    control.middleRows(0, 3) = acc;
    control.middleRows(3, 3) = gyro;
    control.middleRows(6, 4) = Eigen::Vector4f(imu_quat.w(), imu_quat.x(),imu_quat.y(), imu_quat.z());  //w,x,y,z
  ...
  pcl::PointCloud<PointT>::Ptr correct(const pcl::PointCloud<PointT>::ConstPtr& cloud) {
    ...
    init_guess.block<3, 3>(0, 0) = quat().toRotationMatrix();//预测均值中获取四元数
    init_guess.block<3, 1>(0, 3) = pos();//预测均值中获取位姿
