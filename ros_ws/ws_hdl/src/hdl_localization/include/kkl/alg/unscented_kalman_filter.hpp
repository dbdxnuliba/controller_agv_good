...
public:
  ...
  void predict(const VectorXt& control) {
    ...
    ensurePositiveFinite(cov);//保证正定矩阵
    computeSigmaPoints(mean, cov, sigma_points);//计算采样sigma点
    for (int i = 0; i < S; i++) {
      sigma_points.row(i) = system.f(sigma_points.row(i), control);//非线性变换
    }
    ...
    cov_pred.setZero();
    for (int i = 0; i < S; i++) {
      mean_pred += weights[i] * sigma_points.row(i);//加权计算统计均值
    }
    for (int i = 0; i < S; i++) {
      VectorXt diff = sigma_points.row(i).transpose() - mean_pred;
      cov_pred += weights[i] * diff * diff.transpose();//加权计算统计协方差
  ...
  void correct(const VectorXt& measurement) {
    ...
    ext_mean_pred.topLeftCorner(N, 1) = VectorXt(mean);//倒入预测均值
    ext_cov_pred.topLeftCorner(N, N) = MatrixXt(cov);//倒入预测协方差
    ext_cov_pred.bottomRightCorner(K, K) = measurement_noise;

    ensurePositiveFinite(ext_cov_pred);
    computeSigmaPoints(ext_mean_pred, ext_cov_pred, ext_sigma_points);//计算sigma点

    // unscented transform
    expected_measurements.setZero();
    for (int i = 0; i < ext_sigma_points.rows(); i++) {
      expected_measurements.row(i) = system.h(ext_sigma_points.row(i).transpose().topLeftCorner(N, 1));//非线性变换
    ...
    for (int i = 0; i < ext_sigma_points.rows(); i++) {
      expected_measurement_mean += ext_weights[i] * expected_measurements.row(i);//加权计算统计均值
    }
    ...
    for (int i = 0; i < ext_sigma_points.rows(); i++) {
      ...
      expected_measurement_cov += ext_weights[i] * diff * diff.transpose();//加权计算统计协方差
    }
    ...
    kalman_gain = sigma * expected_measurement_cov.inverse();//增益
    const auto& K = kalman_gain;

    VectorXt ext_mean = ext_mean_pred + K * (measurement - expected_measurement_mean);//修正后状态值
    MatrixXt ext_cov = ext_cov_pred - K * expected_measurement_cov * K.transpose();//修正后状态值的协方差
    VectorXt error  = (measurement - expected_measurement_mean);//测量值与估计位置下的观测值之间的误差
    double k_error = sqrt(error[0]*error[0]+error[1]*error[1]+error[2]*error[2]);
    std::cout << "k_error:"<< k_error <<std::endl;
    mean = ext_mean.topLeftCorner(N, 1);//修正后状态值
    cov = ext_cov.topLeftCorner(N, N);//修正后状态值的协方差
...
private:
  ...
  void computeSigmaPoints(const VectorXt& mean, const MatrixXt& cov, MatrixXt& sigma_points) {
    ...
    sigma_points.row(0) = mean;//高斯分布meau: x0处值
    for (int i = 0; i < n; i++) {
      sigma_points.row(1 + i * 2) = mean + l.col(i);//高斯分布右侧采样点 x1处西格玛点
      sigma_points.row(1 + i * 2 + 1) = mean - l.col(i);//高斯分布左侧采样点 x2处西格玛点
    }
  }
