...
sensor_msgs::LaserScanPtr LaserScanKinect::getLaserScanMsg(
  ...
  data.resize(int(scan_msg_->ranges.size()));
  scan_msg_->ranges[0]=0;

  for(int i=0;i<int(scan_msg_->ranges.size());i++){
      if(scan_msg_->ranges[i] ==INFINITY ){     
        scan_msg_->ranges[i]=0;
      }

}

for(int i=2;i<int(scan_msg_->ranges.size());i++){
	 if(isnan(scan_msg_->ranges[i]))
			scan_msg_->ranges[i]= scan_msg_->ranges[i-1];
}
int kk=0;
for(int i=0;i<int(scan_msg_->ranges.size());i++){
	data[i]=scan_msg_->ranges[i];
  	if(data[i]>0)
          	kk++;
...
// && scan_msg_->ranges[i+1]>0 && scan_msg_->ranges[i+2]>0

/*
for(int i=0;i<int(scan_msg_->ranges.size())-20;i++){	
	if(scan_msg_->ranges[i]>0 && scan_msg_->ranges[i+20]==0){
		//std::cout<<i<<std::endl;
		for(int k=0;k<20;k++){
			data[i+k]=0;
		}
...
for(int i=0;i<int(scan_msg_->ranges.size());i++){
	...
	if(scan_msg_->ranges[i]>0 && scan_msg_->ranges[i-1]==0)
		//std::cout<<i<<" ";
		index_.push_back(i);
	if(scan_msg_->ranges[i]>0 && scan_msg_->ranges[i+1]==0)
		//std::cout<<i<<" ";
...
for(auto temp : index_)
	std::cout<<temp<<" ";
std::cout<<std::endl;

int size=index_.size()/2;
for(int i=0;i<size;i++){
	
	if(index_[2*i+1]-index_[2*i+0]<30){
	
		for(int k=index_[2*i+0];k<index_[2*i+1]+1;k++){
			data[k]=0;		
		}
	}
