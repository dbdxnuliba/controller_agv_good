...
void ModelPosition::Update(void)
{
    ...
    switch (localization_mode)
    {
        ...
        case LOCALIZATION_ODOM:
        {
            ...
            {
                //blind zhou add
                double beta = atan(0.5 * tan(goal.a));
                est_pose.x += goal.x / fabs(cos(beta)) * cos(beta + est_pose.a) * dt;
                est_pose.y += goal.x / fabs(cos(beta)) * sin(beta + est_pose.a) * dt;
                est_pose.a += goal.x / fabs(cos(beta)) * tan(goal.a) * dt / wheelbase;
                est_pose.a = normalize(est_pose.a);
            }
            
            // printf("\n%s %d %s: stage dt %.3f command(%.3f %.3f %.3f)\n", __FILE__, __LINE__, __FUNCTION__,
            //         dt, this->goal.x, this->goal.y, this->goal.a);
            // printf("%s %d %s: stage pos(%.3f %.3f %.3f)\n", __FILE__, __LINE__, __FUNCTION__,
            //         est_pose.x, est_pose.y, est_pose.a * 180 / M_PI);
        } break;
...
void ModelPosition::SetSpeed(double x, double y, double a)
{
    ...
    goal.a = a;
    // print_trace();
    // printf("%s %d %s: (%.3f %.3f %.3f)\n", __FILE__, __LINE__, __FUNCTION__, this->goal.x, \
           this->goal.y, this->goal.a);
}
